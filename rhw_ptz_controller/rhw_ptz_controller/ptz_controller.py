...
def _json_find_text(data: Any, key: str) -> str | None:
    if isinstance(data, dict):
        lowered_key = key.lower()
        for current_key, current_value in data.items():
            if str(current_key).lower() == lowered_key and current_value is not None:
                return str(current_value).strip()
        for value in data.values():
            found = _json_find_text(value, key)
            if found:
                return found
    if isinstance(data, list):
        for item in data:
            found = _json_find_text(item, key)
            if found:
                return found
    if isinstance(data, str) and key.lower() == "url":
...
class PtzController:
    ...
    # ==================================================================
    #  手动抓拍 — ISAPI /Streaming/channels/<channelID>/picture
    # ==================================================================

    def get_capture_capabilities(self, *, channel: int = 1) -> Dict[str, object]:
        """获取指定通道的抓拍能力。"""
        normalized_channel = _normalize_channel(channel)
        url = (
            f"{self.base_url}/ISAPI/Streaming/channels/"
            f"{normalized_channel}/picture/async/capabilities?format=json"
        )
        response = self._request(method="GET", url=url)
        output = self._common_result(
            action="get_capture_capabilities",
            response=response,
            extra={"url": url, "channel": normalized_channel},
        )
        if output.get("ok"):
            try:
                output["capabilities_json"] = response.json()
            except ValueError:
                output["raw_body"] = response.text
        return output
    ...
    def capture_picture(
        ...
        *,
        ...
        调用 ISAPI:
          GET /ISAPI/Streaming/channels/<channelID>/picture/async
              ?format=json&imageType=JPEG&URLType=<URLType>&channelFormat=<channelFormat>

        设备能力返回仅支持 imageType=JPEG，响应 JSON 中的 `PictureData.url`
        为存储服务器或设备本地存储的图片地址。
        ...
        dict 包含:
            ok           : bool
            capture_url  : str | None  — `PictureData.url`
            payload_json : dict | None  — 原始 JSON
        """
        normalized_channel = _normalize_channel(channel)
        normalized_image_type = "JPEG"
        normalized_url_type = str(url_type or "localURL").strip() or "localURL"
        if normalized_url_type not in ("cloudURL", "localURL"):
            raise PtzError("url_type must be cloudURL or localURL", category="invalid_capture_url_type")
        normalized_channel_format = str(channel_format or "").strip()

        query_params = {
            "format": "json",
            "imageType": normalized_image_type,
            "URLType": normalized_url_type,
        }
        if normalized_channel_format:
            query_params["channelFormat"] = normalized_channel_format

        url = (
            f"{self.base_url}/ISAPI/Streaming/channels/"
            f"{normalized_channel}/picture/async"
        ...
        try:
            ...
            raise PtzError(f"capture request failed: {exc}", category="request_failed") from exc
        ...
        output: Dict[str, object] = {
            ...
            "url_type": normalized_url_type,
            "channel_format": normalized_channel_format or None,
        ...
        if ok:
            try:
                ...
                output["payload_json"] = payload
                output["capture_url"] = _json_find_text(payload, "url")
                if not output.get("capture_url"):
                    output["capture_url"] = _json_find_text(payload, "URL")
                if not output.get("capture_url") and isinstance(payload, str):
                    normalized_payload = payload.strip()
                    if normalized_payload.startswith(("http://", "https://", "/")):
                        output["capture_url"] = normalized_payload
                if not output.get("capture_url"):
                    output["ok"] = False
                    output["error"] = "capture succeeded but PictureData.url is missing"
                    output["error_category"] = "invalid_capture_payload"
                    if isinstance(payload, dict):
                        output["payload_keys"] = list(payload.keys())
            except ValueError:
                output["ok"] = False
                output["raw_body"] = response.text
                output["error"] = "capture response is not valid JSON"
                output["error_category"] = "invalid_capture_payload"
        else:
            output.update(self._parse_status_body(response))
            output["error"] = f"capture failed with status {response.status_code}"
            output["error_category"] = "downstream_status"

        return output

    def download_capture_url(self, *, capture_url: str) -> Dict[str, object]:
        normalized_url = str(capture_url).strip()
        if not normalized_url:
            raise PtzError("capture_url is required", category="invalid_capture_url")

        if normalized_url.startswith("/"):
            normalized_url = f"{self.base_url}{normalized_url}"
        ...
        try:
            ...
            raise PtzError(f"download capture failed: {exc}", category="capture_download_failed") from exc

        output: Dict[str, object] = {
            "ok": response.status_code in (200, 201, 202, 204),
            "status_code": response.status_code,
            "action": "download_capture_url",
            "capture_url": normalized_url,
            "content_type": response.headers.get("Content-Type", ""),
        }

        if output["ok"]:
            output["image_data"] = response.content
            output["image_size"] = len(response.content)
        else:
            output["image_data"] = None
            output["image_size"] = 0
            output["error"] = f"download failed with status {response.status_code}"
            output["error_category"] = "downstream_status"
            output.update(self._parse_status_body(response))

        return output

    def capture_picture_binary(self, *, channel: int = 1) -> Dict[str, object]:
        """通过同步接口直接抓取 JPEG 图片二进制数据。"""
        normalized_channel = _normalize_channel(channel)
        url = f"{self.base_url}/ISAPI/Streaming/channels/{normalized_channel}/picture"
        ...
        try:
            ...
            raise PtzError(f"capture picture binary failed: {exc}", category="request_failed") from exc

        output: Dict[str, object] = {
            "ok": response.status_code in (200, 201, 202, 204),
            "status_code": response.status_code,
            "action": "capture_picture_binary",
            "url": url,
            "channel": normalized_channel,
            "content_type": response.headers.get("Content-Type", ""),
        }
        ...
        if output["ok"]:
            ...
            output["image_size"] = 0
            output.update(self._parse_status_body(response))
            output["error"] = f"capture binary failed with status {response.status_code}"
            output["error_category"] = "downstream_status"
